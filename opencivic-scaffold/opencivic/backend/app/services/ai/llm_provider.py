...
class LLMProvider(ABC):
    ...
    @abstractmethod
    async def complete(self, system: str, messages: list[dict[str, str]], max_tokens: int = 1000) -> LLMResponse:
...
class OllamaProvider(LLMProvider):
    ...
    async def embed(self, text: str) -> list[float]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{settings.LLM_BASE_URL}/api/embeddings",
...
async def _local_embed(text: str) -> list[float]:
    """Fallback: sentence-transformers local embedding."""
    import asyncio
