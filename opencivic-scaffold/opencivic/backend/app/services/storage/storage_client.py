...
class MinioStorageClient(StorageClient):
    ...
    async def put(self, key: str, data: bytes, content_type: str = "application/octet-stream") -> str:
        ...
        await loop.run_in_executor(None, lambda: self._client().put_object(
    ...
    async def get(self, key: str) -> bytes:
        import asyncio
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(None, lambda: self._client().get_object(
    ...
    async def delete(self, key: str) -> None:
        import asyncio
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, lambda: self._client().delete_object(
            Bucket=settings.MINIO_BUCKET, Key=key
        ))
    ...
    async def presign_get(self, key: str, expires_in: int = 3600) -> str:
        import asyncio
        loop = asyncio.get_event_loop()
        url = await loop.run_in_executor(None, lambda: self._client().generate_presigned_url(
            "get_object", Params={"Bucket": settings.MINIO_BUCKET, "Key": key}, ExpiresIn=expires_in
        ))
        return url
    ...
    async def exists(self, key: str) -> bool:
        ...
        loop = asyncio.get_event_loop()
        try:
            await loop.run_in_executor(None, lambda: self._client().head_object(
                Bucket=settings.MINIO_BUCKET, Key=key
            ))
            return True
        except ClientError:
