...
class TokenExpired(OpenCivicError):
    status_code = 401
    code = "TOKEN_EXPIRED"
...
class UserConflict(OpenCivicError):
    status_code = 409
    code = "USER_CONFLICT"
...
class SchemaInferenceError(OpenCivicError):
    status_code = 422
    code = "SCHEMA_INFERENCE_ERROR"


class SchemaDriftDetected(OpenCivicError):
    status_code = 409
    code = "SCHEMA_DRIFT"


class DatasetDataNotAvailable(OpenCivicError):
    status_code = 404
    code = "DATASET_DATA_NOT_AVAILABLE"
